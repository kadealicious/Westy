...
void wsCameraMakeFPS(wsCamera *camera, mat4 *view_dest, float speed, float pitch_constraint) {
	...
	if(wsInputGetPress(GLFW_KEY_W))
		move_array[FORWARD]++;
	if(wsInputGetPress(GLFW_KEY_S))
		move_array[FORWARD]--;
	if(wsInputGetPress(GLFW_KEY_A))
		move_array[RIGHT]--;
	if(wsInputGetPress(GLFW_KEY_D))
		move_array[RIGHT]++;
	...
	wsCameraMouseMove(camera, wsInputGetMouseMoveX(), wsInputGetMouseMoveY(), pitch_constraint);
}